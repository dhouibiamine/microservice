...
const resolvers = {
  ...
  Mutation: {
    addMovie: async (_, { title, description }) => {
      const client = new movieProto.MovieService(
        "localhost:50051",
        grpc.credentials.createInsecure()
      );
      return new Promise((resolve, reject) => {
        client.createMovie({ title, description }, function (err, response) {
          if (err) {
            reject(err);
          } else {
            resolve(response.movie);
    ...
    addTvShow: async (_, { title, description }) => {
      const client = new tvShowProto.TVShowService(
        "localhost:50052",
        grpc.credentials.createInsecure()
      );
      return new Promise((resolve, reject) => {
        client.CreateTvShow({ title, description }, function (err, response) {
          if (err) {
            reject(err);
          } else {
            resolve(response.tv_show);
